...
from .file_ingestion import FileIngestionProcessor, FileIngestionConfig, load_requirements_from_file
...
class JamaMCPServer:
    ...
    def _setup_handlers(self) -> None:
        ...
        async def handle_list_tools() -> ListToolsResult:
            ...
            return ListToolsResult(
                tools=[
                    ...
                    Tool(
                        name="ingest_requirements_from_file",
                        description="Import and process requirements from file (CSV, JSON, Excel, or text) when Jama Connect is not available",
                        inputSchema={
                            "type": "object",
                            "properties": {
                                "file_path": {
                                    "type": "string",
                                    "description": "Path to requirements file (supports CSV, JSON, Excel, TXT)"
                                },
                                "file_format": {
                                    "type": "string",
                                    "enum": ["csv", "json", "excel", "text", "auto"],
                                    "description": "File format (auto-detected if not specified)",
                                    "default": "auto"
                                },
                                "id_column": {
                                    "type": "string",
                                    "description": "Column name for requirement ID (CSV/Excel only)",
                                    "default": "id"
                                },
                                "name_column": {
                                    "type": "string", 
                                    "description": "Column name for requirement name (CSV/Excel only)",
                                    "default": "name"
                                },
                                "description_column": {
                                    "type": "string",
                                    "description": "Column name for description (CSV/Excel only)",
                                    "default": "description"
                                },
                                "enable_nlp_processing": {
                                    "type": "boolean",
                                    "description": "Process requirements with NLP analysis",
                                    "default": true
                                },
                                "enable_vector_storage": {
                                    "type": "boolean",
                                    "description": "Store in vector database for semantic search",
                                    "default": true
                                }
                            },
                            "required": ["file_path"]
        ...
        async def handle_call_tool(name: str, arguments: dict) -> CallToolResult:
            ...
            try:
                ...
                elif name == "ingest_requirements_from_file":
                    result = await self._handle_ingest_requirements_from_file(arguments)
                else:
    ...
    async def _handle_ingest_requirements_from_file(self, args: Dict[str, Any]) -> Dict[str, Any]:
        """Handle file-based requirement ingestion."""
        file_path = args["file_path"]
        file_format = args.get("file_format", "auto")
        enable_nlp_processing = args.get("enable_nlp_processing", True)
        enable_vector_storage = args.get("enable_vector_storage", True)
        
        logger.info(f"Ingesting requirements from file: {file_path}")
        
        try:
            # Configure file ingestion
            config_kwargs = {}
            if "id_column" in args:
                config_kwargs["id_column"] = args["id_column"]
            if "name_column" in args:
                config_kwargs["name_column"] = args["name_column"]
            if "description_column" in args:
                config_kwargs["description_column"] = args["description_column"]
            
            # Load requirements from file
            requirements = await load_requirements_from_file(
                file_path=file_path,
                file_format=file_format if file_format != "auto" else None,
                **config_kwargs
            )
            ...
            logger.info(f"Loaded {len(requirements)} requirements from file")
            
            # Process with NLP if enabled
            processed_reqs = []
            if enable_nlp_processing and self.nlp_processor:
                logger.info("Processing requirements with NLP...")
                
                batch_data = [
                    (req.description, req.global_id)
                    for req in requirements
                ...
                logger.info(f"Processed {len(processed_reqs)} requirements with NLP")
            
            # Store in vector database if enabled
            vector_stats = {}
            if enable_vector_storage and self.vector_store and processed_reqs:
                logger.info("Storing processed requirements in vector database...")
                
                vector_docs = []
                for processed_req in processed_reqs:
                    if processed_req.embedding is not None:
                        # Find original requirement for metadata
                        original_req = next(
                            (req for req in requirements if req.global_id == processed_req.original_id),
                            None
                        )
                        ...
                        if original_req:
                            metadata.update({
                                "original_type": original_req.item_type,
                                "priority": original_req.priority,
                                "status": original_req.status,
                                "tags": ", ".join(original_req.tags) if original_req.tags else ""
                            })
                ...
                if vector_docs:
                    await self.vector_store.add_documents(vector_docs)
                    vector_stats = await self.vector_store.get_stats()
                    logger.info(f"Stored {len(vector_docs)} requirements in vector database")
            
            # Generate statistics
            business_rules_count = sum(len(req.business_rules) for req in processed_reqs) if processed_reqs else 0
            entities_count = sum(len(req.entities) for req in processed_reqs) if processed_reqs else 0
            ...
            # Analyze requirements types from original data
            file_type_summary = {}
            for req in requirements:
                req_type = req.item_type
                file_type_summary[req_type] = file_type_summary.get(req_type, 0) + 1
            ...
            return {
                ...
                "ingestion_completed": True,
                "statistics": {
                    "total_requirements_loaded": len(requirements),
                    "processed_with_nlp": len(processed_reqs),
                    "business_rules_extracted": business_rules_count,
                    "entities_extracted": entities_count,
                    "file_type_summary": file_type_summary,
                    "nlp_classification_summary": classification_summary
                },
                "vector_storage": {
                    "enabled": enable_vector_storage and self.vector_store is not None,
                    "documents_stored": len(vector_docs) if enable_vector_storage and 'vector_docs' in locals() else 0,
                    "store_stats": vector_stats
                },
                "processing_options": {
                    "nlp_processing_enabled": enable_nlp_processing,
                ...
                "sample_requirements": [
                    {
                        ...
                        "tags": req.tags[:3] if req.tags else []  # Show first 3 tags
                    }
            ...
            logger.error(f"Error ingesting requirements from file: {e}")
            return {
                "error": f"Failed to ingest requirements: {str(e)}",
                "file_path": file_path,
                "file_format": file_format
            }
